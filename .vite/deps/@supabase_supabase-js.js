import { require_browser } from "./chunk-2DBYIXRK.js";
...
var resolveFetch = (customFetch) => {
  ...
  if (customFetch) {
    ...
    _fetch = (...args) =>
      import("./browser-ZAYBH43L.js").then(({ default: fetch2 }) =>
        fetch2(...args),
      );
  } else {
    _fetch = fetch;
...
var PostgrestTransformBuilder = class extends PostgrestBuilder {
  ...
  order(column, { ascending = true, nullsFirst, foreignTable } = {}) {
    ...
    this.url.searchParams.set(
      ...
      `${existingOrder ? `${existingOrder},` : ""}${column}.${
        ...
        nullsFirst === void 0 ? "" : nullsFirst ? ".nullsfirst" : ".nullslast"
      }`,
    );
    return this;
  }
  ...
  rollback() {
    var _a;
    if (
      ((_a = this.headers["Prefer"]) !== null && _a !== void 0 ? _a : "").trim()
        .length > 0
    ) {
  ...
  async send(args, opts = {}) {
    ...
    if (!this._canPush() && args.type === "broadcast") {
      ...
      const options = {
        ...
        headers: {
          apikey:
            (_a = this.socket.accessToken) !== null && _a !== void 0 ? _a : "",
      ...
      try {
        const response = await this._fetchWithTimeout(
          ...
          options,
          (_b = opts.timeout) !== null && _b !== void 0 ? _b : this.timeout,
        );
        if (response.ok) {
          return "ok";
        } else {
...
var RealtimeClient = class {
  ...
  constructor(endPoint, options) {
    ...
    this.reconnectAfterMs = (
      options === null || options === void 0 ? void 0 : options.reconnectAfterMs
    )
  ...
  _sendHeartbeat() {
    ...
    if (this.pendingHeartbeatRef) {
      ...
      (_a = this.conn) === null || _a === void 0
        ? void 0
        : _a.close(WS_CLOSE_NORMAL, "hearbeat timeout");
      return;
    }
...
var resolveResponse = () =>
  __awaiter2(void 0, void 0, void 0, function* () {
    if (typeof Response === "undefined") {
      return (yield import("./browser-ZAYBH43L.js")).Response;
    }
    return Response;
  });
  ...
  __awaiter3(void 0, void 0, void 0, function* () {
    ...
    if (error instanceof Res) {
      error
        ...
        .then((err) => {
          reject(
            new StorageApiError(_getErrorMessage(err), error.status || 500),
          );
        })
        .catch((err) => {
...
function applySettingDefaults(options, defaults) {
  ...
  return {
    ...
    realtime: Object.assign(
      Object.assign({}, DEFAULT_REALTIME_OPTIONS2),
      realtimeOptions,
    ),
    global: Object.assign(
      Object.assign({}, DEFAULT_GLOBAL_OPTIONS2),
      globalOptions,
    ),
  };
}
...
var GoTrueClient = class _GoTrueClient {
  ...
  constructor(options) {
    ...
    if (
      ...
      (_a = this.broadcastChannel) === null || _a === void 0
        ? void 0
        : _a.addEventListener("message", async (event) => {
            this._debug(
  ...
  _debug(...args) {
    if (this.logDebugMessages) {
      this.logger(
        `GoTrueClient@${
          this.instanceID
        } (${version5}) ${/* @__PURE__ */ new Date().toISOString()}`,
        ...args,
      );
    }
    return this;
  ...
  async _exchangeCodeForSession(authCode) {
    const codeVerifier = await getItemAsync(
      this.storage,
      `${this.storageKey}-code-verifier`,
    );
    const { data, error } = await _request(
  ...
  async resend(credentials) {
    try {
      if (
        credentials.type != "email_change" &&
        credentials.type != "phone_change"
      ) {
        await this._removeSession();
      }
  ...
  async __loadSession() {
    ...
    try {
      ...
      const hasExpired = currentSession.expires_at
        ? currentSession.expires_at <= Date.now() / 1e3
        : false;
      this._debug(
        "#__loadSession()",
    ...
    try {
      ...
      return await retryable(
        ...
        (attempt, _, result) =>
          result &&
          result.error &&
          isAuthRetryableFetchError(result.error) && // retryable only if the request can be sent before the backoff overflows the tick duration
          Date.now() + (attempt + 1) * 200 - startedAt <
          ...
          try {
            return await this._useSession(async (result) => {
              ...
              const expiresInTicks = Math.floor(
                (session.expires_at * 1e3 - now) / AUTO_REFRESH_TICK_DURATION,
              );
              this._debug(
...
var SupabaseClient = class {
  ...
  constructor(supabaseUrl, supabaseKey, options) {
    ...
    this.storageKey =
      (_b =
        (_a = settings.auth) === null || _a === void 0
          ? void 0
          : _a.storageKey) !== null && _b !== void 0
        ? _b
        : "";
    this.headers =
      (_d =
        (_c = settings.global) === null || _c === void 0
          ? void 0
